...
1b. NEVER REMOVE any existing skill, tool, technology, or keyword that is already present in the Original CV — even if it seems less relevant to this job. You may REORDER them, but every existing item must still appear somewhere in the output. The CV should only gain relevant additions, never lose existing ATS keywords.
2. From the Job Analysis below, identify which "technical_skills", "technologies", and "keywords" represent the CORE of the role — i.e. they appear central to the job's main responsibilities (e.g. mentioned multiple times, in the title, or in the main responsibilities list), not just mentioned once in passing or as a "nice to have". Add these core items into the Skills section using the EXACT same wording/spelling as in the Job Analysis (ATS scans do exact string matches), under the most fitting existing category — create a new category only if truly necessary.
2b. REORDER the Skills section based on the role's ACTUAL FOCUS as reflected in the Job Analysis as a whole: if the job description's responsibilities and core requirements are primarily ENGINEERING-focused (e.g. debugging, refactoring, code quality, technical investigations, performance, frameworks, version control, collaboration tools), then engineering-related categories/skills must be ordered FIRST — even if a secondary or "nice to have" skill (e.g. a design tool) was also mentioned in the JD. Do not let a single mention of a tangential skill (e.g. Figma, UX) outweigh the overall engineering emphasis of the role. Read the job holistically before reordering.
3. SKIP keywords that are generic, low-impact, or not core to the role — do not stuff irrelevant terms just to pad the list. Quality and relevance over quantity. (This applies only to NEW additions — never to existing CV content, per rule 1b.)
...
8. The CV MUST fit on exactly ONE page — this is non-negotiable. If something must be trimmed to fit, shorten wording/spacing rather than removing existing skills (per rule 1b) or core engineering keywords (per rule 2).
9. FINAL CHECK before output: (a) confirm every skill/keyword from the Original CV is still present somewhere in your draft; (b) scan for any term/phrase repeated 3+ times and rewrite those occurrences with different wording; (c) confirm the overall emphasis/ordering of the CV matches the role's true focus (e.g. an engineering role should read as engineering-focused, not design-focused) as required by rule 2b.
